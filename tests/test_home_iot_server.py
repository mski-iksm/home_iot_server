import pytest
from core.home_iot_server import process_message, detect_device, detect_order, send_ir, make_signal
import yaml
...
class TestDetectDevice:
    ...
    def test_valid_device(self, message, result):
        assert detect_device(message) == result
    ...
    def test_false_device(self, message, result):
        assert detect_device(message) == result
...
class TestDetectOrder:
    ...
    def test_valid_order(self, message, device, result):
        assert detect_order(message, device)[0] == result
...
class TestProcessMessage:
    @pytest.mark.parametrize(
        'message, signal, device, order', [
            ('ライトを付けて', settings['room_light']['orders']
             ['turn_on']['signal'], 'room_light', 'turn_on'),
            ('えーっと電気をつけて', settings['room_light']['orders']
             ['turn_on']['signal'], 'room_light', 'turn_on'),
        ]
    )
    def test_valid_message(self, message, signal, device, order):
        assert process_message(message) == (signal, device, order)
...
class TestSendIR:
    ...
    def test_turn_on_light_send_ir(self, signal):
        assert send_ir(signal).status_code == 200
